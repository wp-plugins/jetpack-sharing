...
class Sharing_Admin {
	...
	public function subscription_menu( $user ) {
		...
		add_submenu_page( 'options-general.php', __( 'Sharing Settings', 'jetpack-sharing' ), __( 'Sharing', 'jetpack-sharing' ), 'publish_posts', 'sharing', array( &$this, 'management_page' ) );
	}
...
<?php
	...
	public function management_page() {
		...
		if ( false == function_exists( 'mb_stripos' ) ) {
			echo '<div id="message" class="updated fade"><h3>' . __( 'Warning! Multibyte support missing!', 'jetpack-sharing' ) . '</h3>';
			echo "<p>" . sprintf( __( 'This plugin will work without it, but multibyte support is used <a href="%s">if available</a>. You may see minor problems with Tweets and other sharing services.', 'jetpack-sharing' ), "http://www.php.net/manual/en/mbstring.installation.php" ) . '</p></div>';
		}

		if ( isset( $_GET['update'] ) && $_GET['update'] == 'saved' )
			echo '<div class="updated"><p>'.__( 'Settings have been saved', 'jetpack-sharing' ).'</p></div>';
?>

	<div class="wrap">
	  	<div class="icon32" id="icon-options-general"><br /></div>
	  	<h2><?php _e( 'Sharing Settings', 'jetpack-sharing' );  ?></h2>
		...
		<div class="share_manage_options">
	  	<h3><?php _e( 'Sharing Buttons', 'jetpack-sharing' )  ?></h3>
	  	<p><?php _e( 'Add sharing buttons to your blog and allow your visitors to share posts with their friends.', 'jetpack-sharing' )  ?></p>

	  	<div id="services-config">
	  		<table id="available-services">
					<tr>
		  			<td class="description">
		  				<h3><?php _e( 'Available Services', 'jetpack-sharing' );  ?></h3>
		  				<p><?php _e( "Drag and drop the services you'd like to enable into the box below.", 'jetpack-sharing' );  ?></p>
		  				<p><a href="#TB_inline?height=395&amp;width=600&amp;inlineId=new-service" title="<?php echo esc_attr( __( 'Add a new service', 'jetpack-sharing' ) );  ?>" class="thickbox" id="add-a-new-service"><?php _e( 'Add a new service', 'jetpack-sharing' );  ?></a></p>
		  			</td>
		  			<td class="services">
						...
						<?php
			  				if ( -1 == get_option( 'blog_public' ) )
								echo '<p><strong>'.__( 'Please note that your services have been restricted because your site is private.', 'jetpack-sharing' ).'</strong></p>';
		  				?>
		  				<br class="clearing" />
		  			</td>
					</tr>
  			...
  			<table id="enabled-services">
  				<tr>
  					<td class="description">
						<h3>
							<?php _e( 'Enabled Services', 'jetpack-sharing' );  ?>
							<img src="<?php echo admin_url( 'images/loading.gif' ); ?>" width="16" height="16" alt="loading" style="vertical-align: middle; display: none" />
						</h3>
						<p><?php _e( 'Services dragged here will appear individually.', 'jetpack-sharing' );  ?></p>
  					</td>
	  				<td class="services" id="share-drop-target">
			  				<h2 id="drag-instructions" <?php if ( count( $enabled['visible'] ) > 0 ) echo ' style="display: none"'; ?>><?php _e( 'Drag and drop available services here.', 'jetpack-sharing' );  ?></h2>
					...
					<td id="hidden-drop-target" class="services">
			  				<p><?php _e( 'Services dragged here will be hidden behind a share button.', 'jetpack-sharing' );  ?></p>
			...
			<table id="live-preview">
				<tr>
					<td class="description">
						<h3><?php _e( 'Live Preview', 'jetpack-sharing' );  ?></h3>
					</td>
					<td class="services">
						<h2<?php if ( count( $enabled['all'] ) > 0 ) echo ' style="display: none"'; ?>><?php _e( 'Sharing is off. Add services above to enable.', 'jetpack-sharing' );  ?></h2>
						<div class="sharedaddy sd-sharing-enabled">
							...
							<div class="sd-content">
								<ul class="preview">
					                ...
					                <li class="advanced"><a href="#" class="sharing-anchor sd-button share-more"><span><?php _e( 'More', 'jetpack-sharing' );  ?></span></a></li>
					                <?php endif; ?>
					            </ul>
	  	...
	  	<form method="post" action="">
	  		<table class="form-table">
	  			<tbody>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Button style', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<select name="button_style" id="button_style">
	  							<option<?php if ( $global['button_style'] == 'icon-text' ) echo ' selected="selected"';?> value="icon-text"><?php _e( 'Icon + text', 'jetpack-sharing' );  ?></option>
	  							<option<?php if ( $global['button_style'] == 'icon' ) echo ' selected="selected"';?> value="icon"><?php _e( 'Icon only', 'jetpack-sharing' );  ?></option>
	  							<option<?php if ( $global['button_style'] == 'text' ) echo ' selected="selected"';?> value="text"><?php _e( 'Text only', 'jetpack-sharing' );  ?></option>
	  							<option<?php if ( $global['button_style'] == 'official' ) echo ' selected="selected"';?> value="official"><?php _e( 'Official buttons', 'jetpack-sharing' );  ?></option>
	  						</select>
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Sharing label', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_label" value="<?php echo esc_attr( $global['sharing_label'] ); ?>" />
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row"><label><?php _e( 'Open links in', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<select name="open_links">
	  							<option<?php if ( $global['open_links'] == 'new' ) echo ' selected="selected"';?> value="new"><?php _e( 'New window', 'jetpack-sharing' );  ?></option>
	  							<option<?php if ( $global['open_links'] == 'same' ) echo ' selected="selected"';?> value="same"><?php _e( 'Same window', 'jetpack-sharing' );  ?></option>
	  						</select>
	  					</td>
	  				</tr>
	  				<?php echo apply_filters( 'sharing_show_buttons_on_row_start', '<tr valign="top">' ); ?>
	  					<th scope="row"><label><?php _e( 'Show buttons on', 'jetpack-sharing' );  ?></label></th>
	  					<td>
						<?php
							$br = false;
							foreach ( $shows as $show ) :
								if ( 'index' == $show ) {
									$label = __( 'Front Page, Archive Pages, and Search Results', 'jetpack-sharing' );
								} else {
		  	...
		  	<p class="submit">
					<input type="submit" name="submit" class="button-primary" value="<?php _e( 'Save Changes', 'jetpack-sharing' );  ?>" />
				</p>
	  ...
	  <div id="new-service" style="display: none">
	  	<form method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" id="new-service-form">
	  		<table class="form-table">
	  			<tbody>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Service name', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_name" id="new_sharing_name" size="40" />
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Sharing URL', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_url" id="new_sharing_url" size="40" />

	  						<p><?php _e( 'You can add the following variables to your service sharing URL:', 'jetpack-sharing' );  ?><br/>
	  						<code>%post_title%</code>, <code>%post_url%</code>, <code>%post_full_url%</code>, <code>%post_excerpt%</code>, <code>%post_tags%</code></p>
	  					</td>
	  				</tr>
	  				<tr valign="top">
	  					<th scope="row" width="100"><label><?php _e( 'Icon URL', 'jetpack-sharing' );  ?></label></th>
	  					<td>
	  						<input type="text" name="sharing_icon" id="new_sharing_icon" size="40" />
	  						<p><?php _e( 'Enter the URL of a 16x16px icon you want to use for this service.', 'jetpack-sharing' );  ?></p>
	  					</td>
	  				</tr>
	  				<tr valign="top" width="100">
	  					<th scope="row"></th>
	  					<td>
								<input type="submit" class="button-primary" value="<?php _e( 'Create Share Button', 'jetpack-sharing' );  ?>" />
	  						<img src="<?php echo admin_url( 'images/loading.gif' ); ?>" width="16" height="16" alt="loading" style="vertical-align: middle; display: none" />
	  					</td>
	  				</tr>
		...
		<?php do_action( 'post_admin_screen_sharing' ) ?>
				...
				<div class="inerror" style="display: none; margin-top: 15px">
					<p><?php _e( 'An error occurred creating your new sharing service - please check you gave valid details.', 'jetpack-sharing' );  ?></p>
				</div>
